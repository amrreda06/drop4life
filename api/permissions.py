...
from .role_utils import is_api_allowed_for_role, is_public_api_path, normalize_role_code
...
class RoleBasedPermission(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if not is_api_allowed_for_role(request.path, account.role_code or account.role):
...
class IsSuperAdmin(BasePermission):
    message = 'فقط المسؤول الأعلى يمكنه تنفيذ هذا الإجراء.'

    def has_permission(self, request, view):
        account = get_request_account(request)
        return account is not None and normalize_role_code(account.role_code or account.role) == 'DR'


class IsSuperAdminOrAdmin(BasePermission):
    message = 'فقط المسؤول الأعلى أو الأدمن يمكنه تنفيذ هذا الإجراء.'

    def has_permission(self, request, view):
        account = get_request_account(request)
        return account is not None and normalize_role_code(account.role_code or account.role) in ('DR', 'ADM')
...
class IsSuperAdminWriteOnly(BasePermission):
    ...
    message = 'فقط المسؤول الأعلى يمكنه تعديل أو حذف السجلات.'
